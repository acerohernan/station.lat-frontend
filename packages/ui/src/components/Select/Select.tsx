...
import React, { forwardRef, type ComponentProps, ReactElement, useState } from "react";
...
const Select = forwardRef<HTMLInputElement, InputProps>(
  ({ id, label, variant, bottomMessage, ...props }, forwardedRef) => {
    ...
    return (
      <div>
        <div className="group relative w-full">
          ...
          <div className="absolute flex top-0 bottom-0 right-0 items-center justify-center mr-1">
            <ChevronDownIcon className={`w-5 h-5 mr-2 transition-all tran ${isOpen ? "rotate-180" : "rotate-0"}`} />
          </div>
        </div>
...
export const Select2 = () => {
  ...
  return (
    <div className="peer w-full bg-transparent px-3 py-3 outline-none transition-colors dark:text-white relative text-gray_dark group-focus-within:!text-blue_primary border-blue_light group-hover:border-blue_primary/50 dark:border-white/30 dark:group-hover:border-white/50 group-focus-within:!border-blue_primary">
      America/Bogota -5:00
      <div className="absolute flex top-0 bottom-0 right-0 items-center justify-center mr-1">
        <ChevronDownIcon className={`w-5 h-5 mr-2 transition-all tran ${isOpen ? "rotate-180" : "rotate-0"}`} />
      </div>
